...
class Customer {
    ...
    get info() {
        return `Nombre: ${this.name}; Correo electrónico: ${this.email}`;
    }
}
...
class Reservation {
    ...
    get info() {
        return `Fecha y hora reserva: ${this.date.toLocaleString()}, Cliente: ${this.customer.info}, Comensales: ${this.guests}`;
    }

    static validateReservation(date, guests) {
        const now = new Date();
        return date > now && guests > 0;
    }
}
...
document.getElementById("reservation-form").addEventListener("submit", function (event) {
    ...
    if (Reservation.validateReservation(date, guests)) {
...
const customer1 = new Customer(1, "Jorge Cruz", "georg@gmail.com");
